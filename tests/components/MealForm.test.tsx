...
describe(MealForm, () => {
  describe('when not passed a meal', () => {
    test.todo('it adds a new meal to the database', async () => {
      ...
      const { getByLabelText, getByRole } = render(<MealForm ingredientOptions={[ingredient1, ingredient2]}/>)
      
      await act(async () => {
        fireEvent.change(getByLabelText("Meal Name:"), {target: {value: "testMealName"}})
      })

      await act(async () => {
        fireEvent.change(getByLabelText("Date Eaten:"), {target: {value: new Date('2023-09-15')}})
      })
