...
import { getIngredients, getMealWithIngredients, getMeals } from "@/utils/dbQueries";
  ...
  describe('when user is signed in', () => {
    ...
    describe('when meal exists but not associated with the user', () => {
      test.todo('throws an error', async () => {
        ...
        await expect(async () => await getMealWithIngredients(mealId)).rejects.toThrow()
      })
    })
    ...
    describe('when meal does not exist', () => {
      test.todo('throws an error', async () => {
        mocks.getUserByClerkId.mockReturnValueOnce(currentUser)
        await expect(async () => await getMealWithIngredients('nonExistentMeal')).rejects.toThrow()
      })
    })
  })
})
